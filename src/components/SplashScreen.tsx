...
const SplashScreen: React.FC<{ faction?: Faction, show?: boolean, onClose?: () => void, api?: SpringRef }> = ({ faction, show, onClose, api }) => {
    ...
    return (
        <Backdrop open={showBackdrop} sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }} onClick={onClose}>
            <Stack justifyContent={"center"} alignItems="center" direction={{ xs: "column", md: "row" }}>
                <AnimatedText fontFamily={faction === "fartian" ? "Under Authority" : "Bebas"} fontSize={{ xs: 46, md: 72 }} textTransform="capitalize" hide={!show} text={`${faction}`} onRest={onRest} />

                <Box sx={{ width: { xs: 200, md: 320 } }} textAlign="center">

                    {imageSpring(({ opacity }, item) =>
                        <animated.img src={item} alt="splash screen" style={{
                            opacity: opacity,
                            width: "100%",
                            height: "100%",
                            ...imageSpring,
